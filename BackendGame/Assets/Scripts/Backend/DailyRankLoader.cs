...
public class DailyRankLoader : MonoBehaviour
{
    ...
    [SerializeField]
    private DailyRankData myRankData;   // 내 랭킹 정보를 출력하는 UI 오브젝트
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        for(int i = 0; i < rankerCount; i++)
                        {
                            ...
                            // 닉네임은 별도로 설정하지 않은 유저도 존재할 수 있기 떄문에
                            // 닉네임이 존재하지 않는 유저는 닉네임 대신 gamerId를 출력
                            rankDataList[i].Nickname = rankDataJson[i].ContainsKey("nickname") ?
                                rankDataJson[i]["nickname"]?.ToString() : UserInfo.Data.gamerId;
                        }

                        // 만약 rankerCount가 20위까지 존재하지 않으면 나머지 랭킹에는 빈 데이터를 생성
    ...
    private void GetMyRank()
    {
        ...
        {
            // 닉네임이 없으면 gamerId, 있으면 nickname 사용
            ...
            if(callback.IsSuccess())
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        myRankData.Nickname = rankDataJson[0].ContainsKey("nickname") ?
