...
public class Nickname : LoginBase
{
    ...
    [SerializeField]
    private Image iamgeNickname;                // 닉네임 필드 색상 변경
    [SerializeField]
    ...
    public void OnClickUpdateNickname()
    {
        ...
        // 필드 값이 비버있는지 체크
        if(IsFieldDataEmpty(iamgeNickname, inputFieldNickname.text, "Nickname")) return;
    ...
    private void UpdateNickname()
    {
        ...
        Backend.BMember.UpdateNickname(inputFieldNickname.text, callback =>
        {
            ...
            if(callback.IsSuccess())
            {
                ...
                // 닉네임 변경에 성공했을 때 onNicknameEvent에 등록되어 있는 이벤트 메서드 호출
            ...
            else
            {
                ...
                GuideForIncorrectlyEnteredData(iamgeNickname, message);
            }
        });
    }
}
