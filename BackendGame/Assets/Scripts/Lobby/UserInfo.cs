...
public class UserInfo : MonoBehaviour
{
    ...
    public void GetUserInfoFromBackend()
    {
        ...
        Backend.BMember.GetUserInfo(callBack =>
        {
            ...
            // 유저 정보 불러오기가 완료됐을 때 onUserInfoEvent에 등록되어 있는 이벤트 메소드 호출
...
public class UserInfoData
{
    ...
    public string countryCode;              // 국가 코드를 설정하지 않은 경우 null
    public string nickname;                 // 닉네임을 설정하지 않은 경우 null
    public string inDate;                   // 유저의 inDate
    public string emailForFindPassword;     // 커스텀 계정 id, pw 찾기 용 이메일. 등록하지 않으면 null
