...
public class FindPW : LoginBase
{
	...
	[SerializeField]
	private	TMP_InputField inputFieldEmail; // E-mail 텍스트 정보 추출
	...
	public void OnClickFindPW()
	{
        ...
        {
            GuideForIncorrectlyEnteredData(imageEmail, "메일 형식이 올바르지 않습니다. (ex. address@xx.xx)");
			return;
        }
	...
	private void FindCustomPW()
	{
		...
		{
			// "비밀번호 찾기" 버튼의 상호작용 활성화
			btnFindPW.interactable = true;

			// 메일 발송 성공
			if (callback.IsSuccess())
			{
				SetMessage($"{inputFieldEmail.text} 주소로 메일을 발송했습니다.");
			}
			// 메일 발송 실패
			else
			{
